...
import { addContact, updateContact } from "../store/redux/contact/contact.action";

const Create = ( props ) => {
  const dispatch = useDispatch();
  const history = useHistory()
  const { contact: { data } } = useSelector((state) => state);
  ...
  useEffect(() => {
    ...
    try {
      let getContactId = params.id || null;
      let getContact = data.filter(item => item.id === getContactId);
      console.log("c", props)
      _setForm({
        id: getContact[0].id,
    ...
    <div className="contact-form">
      ...
      <form onSubmit={(e) => _isUpdate ? handleUpdate(e) : handleSubmit(e)}>
        <div className="form-group">
          ...
          <input type="text" className="control-input" value={_form.name} onChange={(e) => handleChange('name', e.target.value)}/>
        </div>
        <div className="form-group">
          <label className="control-label">Phone Number</label>
          <input type="text" className="control-input" value={_form.number} onChange={(e) => handleChange('number', e.target.value)}/>
        </div>
        <div className="form-group">
          <label className="control-label">Description</label>
          <textarea type="textarea" className="control-input" value={_form.desc} onChange={(e) => handleChange('desc', e.target.value)}/>
        </div>
